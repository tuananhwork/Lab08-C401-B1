...
> Trong lần chạy đánh giá chính thức, lớp retrieval không trả về chunks cho bất kỳ câu hỏi nào do `get_embedding()` ném `NotImplementedError`. Kết quả là cả baseline và variant đều trả về ABSTAIN trên toàn bộ 10 câu hỏi, nên so sánh A/B chưa có giá trị thực tế.
> Variant này vẫn là lựa chọn tuning chính dựa trên phân tích lỗi và thiết kế pipeline, nhưng cần sửa lỗi embedding và rebuild index để xác thực hiệu quả.
